...
private:
	...
	double m_wieght_vec[OBJ_NUM]; //权重向量
	int    m_neighbor[T];
public:
	Individual();      //构造函数
	...
	void SetWeightVec(double *vec, int obj=OBJ_NUM);//设置个体对应的权重向量
	void SetNeighbors(int *neighbors, int num=T);
...
void Individual::SetWeightVec(double *vec ,int obj/* =OBJ_NUM */){memcpy(m_wieght_vec, vec, obj*sizeof(double));} 
void Individual::SetNeighbors(int *neighbors, int num/* =T */){memcpy(m_neighbor, neighbors, sizeof(int)*num );}
