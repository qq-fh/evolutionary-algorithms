...
stim = time.time()
#-----------------------执行程序----------------------------
genration_num = 50  #迭代次数
n = 30                 #变量维数
Func_Num = 3          #进行测试的函数编号
...
Population = MOEAD.getActualInitSol(N, boudList)  #随机初始化种群
...
for iteration in range(genration_num):   #迭代次数
    mergePopu = np.concatenate( (Population, Population2) , axis=0)     #父代子代群体混合
    
    vals = Func[Func_Num](mergePopu)
...
#    mutationPopu = GA.mutation(crossoverPopu, Pm=1/np.sum(encodeLengths))  #位点变异
    ...
    crossoverPopu = MOEAD.actualCrossover2(Population, boudList, Pc=0.9)
    mutationPopu = MOEAD.polyMutation2(crossoverPopu, boudList)
...
valsDisplay =Func[Func_Num](Population)
